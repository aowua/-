...
HAL_StatusTypeDef SERVO_Send_recv(MOTOR_send *pData, MOTOR_recv *rData)
{
	...
	// SET_485_DE_UP();
	// SET_485_RE_UP();
    // HAL_UART_Transmit(&UART1_Handler, (uint8_t *)pData, sizeof(pData->motor_send_data), 10); 
		

	// SET_485_RE_DOWN();
	// SET_485_DE_DOWN();
	// HAL_UART_Receive(&UART1_Handler, (uint8_t *)rData, sizeof(rData->motor_recv_data), 10);		
    RS485_Send_Data((uint8_t *)pData, sizeof(pData->motor_send_data));
	
//    RS485_Receive_Data((uint8_t *)rData, sizeof(rData->motor_recv_data));
